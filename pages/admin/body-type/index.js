...
export default function Index() {
    ...
    const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_URL}admin/body-type-list`, fetcher);
    ...
    return (
        <>
            <Head>
                <title>LR | Body Type   </title>
                <meta name="description" content="Generated by create next app" />
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <link rel="icon" href="../../img/favicon.ico" />
            </Head>
            <Layout>
                <Breadcrumb title="Body Type" addBtn="Add Body Type" url="/admin/body-type/add-body" />
                                ...
                                {data.data.map((body, i) => {
                                    return (
                                        <tr key={i}>
                                            <td>  {i + 1} </td>
                                            <td>
                                                <img src={`${process.env.NEXT_PUBLIC_URL}${body.body_image}`} width={70} />
                                            </td>
                                            <td> {body.body_name} </td>
                                            <td> {body.body_slug} </td>
                                            <td> {body.sorting} </td>
                                            <td>
                                                <button className={`active small-btn ${body.body_status === 1 ? 'active' : 'inactive'} `} onClick={() => updateStatus(body.body_status === 1 ? 2 : 1, body._id)}>{body.body_status === 1 ? 'active' : 'in-active'} </button>
                                            </td>
                                            <td> {dateFormat(body.createdAt)} </td>
                                            <td> {dateFormat(body.updatedAt)} </td>
                                            <td>
                                                <div className="table-buttons">
                                                    <Link href={`/admin/body-type/add-body?id=${body._id}`}> <button className='duplicate'>Edit</button></Link>
                                                </div>
                                            </td>
                                        </tr>
                                    )

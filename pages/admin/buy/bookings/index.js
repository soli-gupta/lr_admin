...
function BookingList() {
    ...
    const { data, error } = useSWR(`${Url}admin/get-all-orders?page=${page}&orderType=buy`, fetcher);
    ...
    const viewOrderDetails = (_id) => {
        router.push(`/admin/buy/bookings/${_id}`);
    }
                                ...
                                data !== undefined && data.orders.map((order, i) => {
                                    return (
                                        <tr key={order._id}>
                                            ...
                                            <td>{order.order_car_name}</td>
                                            <td>{order.order_car_fuel_type}</td>
                                            <td>{order.order_car_location}</td>
                                            <td>
                                                <button className={`small-btn ${order.order_status === 1 ? 'in-stock' : order.order_status === 2 ? 'active' : order.order_status === 3 ? "red" : order.order_status === 4 ? 'pending' : ''}`}>
                                                    {order.order_status == 1 ? 'Booked' : order.order_status === 2 ? 'Completed' : order.order_status === 3 ? "Cancelled" : order.order_status === 4 ? 'Pending' : ''}
                                                </button>
                                            </td>

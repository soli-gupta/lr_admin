...
function Index() {
    ...
    const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_URL}admin/specification-category-list?page=${page}`, fetcher);
    ...
    const viewSpecificationDetails = (id) => {
        router.push('/admin/specification-category/' + id);
    }
    ...
    return (
        <>
            ...
            <Layout>
                <Breadcrumb title="Feature & Specifications" addBtn="Add Specification" url="/admin/specification-category/add-specification" />
                ...
                <Paginationn page={page} const rowCount={data !== undefined ? data.data.length : ''} />
            </Layout>
        </>
    )
}

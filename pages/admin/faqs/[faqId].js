...
function EditFAQ() {
    ...
    const { data, error } = useSWR(`${Url}admin/fetch-faq/${router.query.faqId}`, fetcher);
    ...
    const updateFAQ = async (e) => {
        ...
        if (e.target.faq_description.value.length <= 0) {
            document.getElementById('faqDescription').innerHTML = 'Please enter slug!';
        ...
        await axios.patch(`${Url}admin/update-faq/${router.query.faqId}`, formData, {
            headers: {
                token: localStorage.getItem('lr-admin-token')
            }
        }).then((response) => {
    ...
    if (!data.faq) return 'Loading...'
    let faq = data.faq;
    return (
        <>
            <AdminHead pageTitle={`Edit FAQ`} />
            <Head>
            ...
            <Layout>
                <Breadcrumb title="LR | Edit FAQ " />
                <div className="main-data">

                    <form method="POST" onSubmit={updateFAQ} encType="multipart/form-data" id="faq-form">
                        <div className="form-style">
                            <div className="wrapper">
                                <div className="form-row two-col">
                                        ...
                                        <select className="floating-select" id="faq_type" name='faq_type' onChange={manageEventHandler}>
                                            <option value=""></option>
                                            {
                                                getCmsPages && getCmsPages.map((cms) => {
                                                    return (
                                                        <option value={`${cms.slug}`} key={cms._id} selected={faq.faq_type === cms.slug ? true : false}>{cms.name}</option>
                                                    )
                                                })
                                            }
                                        </select>
                                    ...
                                    <div className="form-div">
                                        <input id="faq_question" name="faq_question" type="text" placeholder=" " defaultValue={faq.faq_question} onChange={manageEventHandler} />
                                ...
                                <div className="form-row one-col">
                                    ...
                                    <div className="form-div">
                                        <textarea className="summernote" onChange={manageEventHandler} name="faq_description" id="faq_description" defaultValue={faq.faq_description}></textarea>

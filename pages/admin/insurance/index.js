...
function Index() {
    ...
    const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_URL}admin/get-all-insurances?page=${page}`, fetcher);
    ...
    const updateStatus = async (status, id) => {
        ...
        axios.get(`${process.env.NEXT_PUBLIC_URL}admin/block-unblock-insurance?id=${id}&status=${status}`, {
            headers: {
                token: localStorage.getItem('lr-admin-token')
            }
        }).then((response) => {
    ...
    return (
        <>
            ...
            <Layout>
                <Breadcrumb title="Insurance" addBtn="Add Insurance" url="/admin/insurance/add-insurance" />
                <div className="main-data">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            {
                                data !== undefined && data.insurances.map((insurance, i) => {
                                    return (
                                        <tr key={insurance._id}>
                                            ...
                                            <td>
                                                <img src={`${process.env.NEXT_PUBLIC_URL}${insurance.image}`} width={80} alt={insurance.name} />
                                            </td>

                                            <td>{insurance.name}</td>
                                            {/* <td> <a href="#"> {insurance.slug}</a></td> */}
                                            <td><button className={`small-btn ${insurance.status === 1 ? 'active' : 'inactive'}`} onClick={() => updateStatus(insurance.status == 1 ? 2 : 1, insurance._id)}   >{insurance.status == 1 ? 'Active' : 'In-active'}</button></td>
                                            <td>{dateFormat(insurance.createdAt)}</td>
                                            <td>{dateFormat(insurance.updatedAt)}</td>
                                            <td>
                                                <div className="table-buttons">
                                                    <button className='duplicate' onClick={(e) => viewInsuranceDetails(insurance._id)}>Edit</button>
                    ...
                    <Paginationn page={page} const rowCount={data !== undefined ? data.insurances.length : ''} />
                </div>
            </Layout>
        </>
    )

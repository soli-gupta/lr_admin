...
export default function Index() {
    ...
    const addUpdateServiceCategory = async (e) => {
        ...
        if (e.target.car_care_category_name.value.length <= 0) {
        ...
        if (e.target.car_care_category_slug.value.length <= 0) {
        ...
        const addUrl = `${process.env.NEXT_PUBLIC_URL}admin/create-car-care-category`
        const updateUrl = `${process.env.NEXT_PUBLIC_URL}admin/update-car-care-category/${query}`
        ...
        if (query) {
            await axios.post(updateUrl, formData, {
                ...
                if (res.data.status == 1) {
                    router.push('/admin/car-care/car-care-category')
                }
    ...
    const editServiceCategory = async () => {
        axios.get(`${process.env.NEXT_PUBLIC_URL}admin/view-car-care-category/${query}`, {
            headers: {
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-admin-token") : false,
            }
        }).then(function (res) {
    ...
    return (
        <>
            <Head>
                <title>LR | {query ? 'Update' : 'Add'} Car Care Category </title>
            ...
            <Layout>
                <Breadcrumb title={query ? 'Update Car Care Category' : ' Add Car Care Category'} />
                        ...
                        <div className="wrapper">
                            <div className="form-row">
                                ...
                                <div className="form-div">
                                    ...
                                    <input id="serviceCategoryName" placeholder=' ' name='car_care_category_name' type="text" defaultValue={data && data.name ? data.name : ''} onChange={(e) => eventChangeHandler(e)} onKeyUp={(e) => createSlug(e.target.value, 'serviceCategorySlug')} onLoad={(e) => createSlug(e.target.value, 'serviceCategorySlug')} />
                                    <label htmlFor="serviceCategoryName">Service Category Name</label>
                                    <small id="serviceCategoryNameError" className="error"></small>
                                </div>

                                <div className="form-div">
                                    <input id="serviceCategorySlug" placeholder=' ' name="car_care_category_slug" type="text" defaultValue={data && data.slug ? data.slug : ''} onChange={(e) => eventChangeHandler(e)} />
                            ...
                            <div className='form-row'>
                                <textarea id="summernote" name="car_care_category_description" defaultValue={data && data.description ? data.description : ''}>
                                </textarea>
                            </div>
                            <div className='form-row two-col'>
                                <div className="form-div">
                                    <input id="sort" name="car_care_category_sorting" type="text" placeholder=" " defaultValue={data && data.sorting ? data.sorting : ''} />

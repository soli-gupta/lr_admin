...
export default function Index() {
    ...
    const addUpdateServiceDiscount = async (e) => {
        ...
        if (e.target.car_care_discount_price.value.length <= 0) {
        ...
        // setTinyloader(true);
        const formData = new FormData(document.getElementById('addCarCareDiscountForm'));

        const addUrl = `${process.env.NEXT_PUBLIC_URL}admin/create-car-care-discount`
        const updateUrl = `${process.env.NEXT_PUBLIC_URL}admin/update-car-care-discount/${id}`
        ...
        if (id) {
            await axios.post(updateUrl, formData, {
                ...
                if (res.data.status == 1) {
                    router.push('/admin/car-care/car-care-discount')
                }
            }).catch(function (error) {
                setTinyloader(false)
                console.log(error)
    ...
    const editServiceCategory = async () => {
        axios.get(`${process.env.NEXT_PUBLIC_URL}admin/view-car-care-discount/${id}`, {
            headers: {
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-admin-token") : false,
            }
        }).then(function (res) {
        ...
        <>
            ...
            <Layout>
                ...
                <div className="form-style calender-style">
                    <form action="" id="addCarCareDiscountForm" onSubmit={(e) => addUpdateServiceDiscount(e)} method="POST">
                        <div className="wrapper">
                            ...
                            <div className="form-row">
                                ...
                                <div className="form-div">
                                    <input id="car_care_discount_price" name="car_care_discount_price" type="text" placeholder=" " defaultValue={data && data.car_care_discount_price ? data.car_care_discount_price : ''} />
